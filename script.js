document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (validateCurrentSection()) {
            ...
            const flatData = {
                ...
                "Segmento": "Proyecto_Academico",
                "Pregunta 1": answers.p1 || "",
                "Pregunta 2": answers.p2 === "Otra" ? `Otra: ${answers.p2_otra || ""}` : (answers.p2 || ""),
                "Pregunta 3": answers.p3 || "",
                "Pregunta 4": answers.p4 === "Otra" ? `Otra: ${answers.p4_otra || ""}` : (answers.p4 || ""),
                "Pregunta 5": answers.p5 || "",
                "Pregunta 6": answers.p6 === "Otra" ? `Otra: ${answers.p6_otra || ""}` : (answers.p6 || ""),
                "Pregunta 7": answers.p7 || "",
                "Pregunta 8": answers.p8 ? (answers.p8 === "Otra" ? `Otra: ${answers.p8_otra || ""} - Motivo: ${answers.p8_reason || ""}` : (answers.p8 + (answers.p8_reason ? ' - Motivo: ' + answers.p8_reason : ''))) : "",
                "Pregunta 9": Array.isArray(answers.p9) ? answers.p9.join(', ') : (answers.p9 || ""),
                "Pregunta 10": answers.p10 || "",
                "Pregunta 11": answers.p11 === "Otra" ? `Otra: ${answers.p11_otra || ""}` : (answers.p11 || ""),
    ...
    function validateCurrentSection() {
        ...
        questionGroups.forEach(group => {
            ...
            // Validar textarea si es visible y requerido (para P8 y P12)
    ...
    // LÓGICA CONDICIONAL: Genérica para opciones "Otra"
    const radioGroupsWithOtra = ['p2', 'p4', 'p6', 'p8', 'p11'];
    radioGroupsWithOtra.forEach(groupName => {
        const radios = document.querySelectorAll(`input[name="${groupName}"]`);
        const otraWrap = document.getElementById(`q${groupName.replace('p', '')}-otra`);
        if (otraWrap) {
            radios.forEach(radio => {
                radio.addEventListener('change', (e) => {
                    if (e.target.value === 'Otra') {
                        otraWrap.classList.remove('hidden');
                    } else {
                        otraWrap.classList.add('hidden');
                        const input = otraWrap.querySelector('input');
                        if (input) input.value = '';
                        // Remove error when hiding
                        const group = otraWrap.closest('.question-group');
                        if (group) group.classList.remove('error');
                    }
                });
            });
